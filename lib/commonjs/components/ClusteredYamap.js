...
const YaMapNativeComponent = (0, _reactNative.requireNativeComponent)('ClusteredYamapView');
class ClusteredYamap extends _react.default.Component {
  static defaultProps = {
  ...
  findMasstransitRoutes(points, callback) {
    this._findRoutes(points, _Yamap.YaMap.ALL_MASSTRANSIT_VEHICLES, callback);
  }
  ...
  getCommand(cmd) {
    return _reactNative.Platform.OS === 'ios' ? _reactNative.UIManager.getViewManagerConfig('ClusteredYamapView').Commands[cmd] : cmd;
  }
  processRoute(event) {
    const {
      id,
